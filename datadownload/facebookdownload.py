#This is the function to download all the facebook data and dump all of them into JSON file and 
#MySQL tables
import facebook
import requests
import time
...
delta_days=5 #after inital load, every time this run is to load up to 3 sdays of data
#it is to get total posts of a business
#it returns total count as well as all the post contentsf
def get_totalpost(business):
    ...
    posts = graph.get_connections(profile['id'], 'posts?since='+str(unix_time)) #get the post since a time
